...
def pci_stationarity_plots(filter_obj, ydata, title='',ndens=1000,split_orders=False,nchan=None,zero_mean=False,detrend=False):
    '''
    ...
    if split_orders:
        ...
        plotting.plotconfig(lbsize=20, lgsize=16, fsize=18, 
                            ...
                            fontfamily = 'STIXGeneral')
        nf=len(filters)
        fig1, axs = plt.subplots(nrows=order+2,ncols=len(filters),squeeze=False,figsize=[6.4*nf, 4.8*((order+2))],sharex="col",sharey="row")
        for ax in axs.flat:
            ax.label_outer()
...
def stationarity_plots(channel_sets, title='',ndens=1000,split_orders=False,nchan=None):
    '''
    Make time-domain plots to explore PCI channel stationarity

    Arguments:
       channel_sets list(ndarray)  Each array of lenght of the number of channels
    Optional:
       title        str       Plot title
       ndens        int       target number of samples to show in each plot curve
                              or 0 for all samples
    ...
    plotting.plotconfig(lbsize=20, lgsize=16, fsize=18, 
                    ticklabelsize=20, style='publication',
                    fontfamily = 'STIXGeneral')
    fig1, axs = plt.subplots(ncols=len(sets),squeeze=False,sharey=True)
    axs=axs[0]
...
def pci_temporal_variance_corr_plots(filter_obj, ydata, title='',ndens=1000,nchan=None,zero_mean=False,detrend=False):
    filters=filter_obj
    if not isinstance(filters,list):
        filters=[filters]
    if nchan is None:
    ...
    sets=[np.array(xf.apply_for_channels(ydata, n_channels=nchan,zero_mean=zero_mean,detrend=detrend)) for xf in filters]
    temporal_variance_corr_plots(sets, title='',ndens=ndens,nchan=nchan)

def temporal_variance_corr_plots(channel_sets, title='',ndens=1000,nchan=None):
    sets=channel_sets
    if not isinstance(sets,list):
        sets=[sets]
    if not isinstance(title,list):
    ...
    plotting.plotconfig(lbsize=20, lgsize=16, fsize=18, 
                    ticklabelsize=20, style='publication',
                    fontfamily = 'STIXGeneral')
    fig1, axs1 = plt.subplots(ncols=len(sets),nrows=1,squeeze=False,sharey=True,figsize=[6.4*nf, 4.8*((1))])
    fig1, axs2 = plt.subplots(ncols=len(sets),nrows=nplot-1,squeeze=False,sharey=True,figsize=[6.4*nf, 4.8*((nplot))])
    ...
    for i,x in enumerate(sets):
        ...
        binned=x[:nchan,:ntrim].reshape(nchan,-1,nbatch)
        if True:
            print("subtracting bin means!")
            binned=(binned.T-binned.mean(axis=2).T).T
        print('shape of input,trimmed,binned',x.shape,x[:,:ntrim].shape,binned.shape)
        ...
        for j in range(nplot):
            ...
            #print(stdevs[j,::ev].shape)
            ax.semilogy(stdevs[j,::ev], marker='.',linewidth=1, label=r'$\mathrm{ch}$[' + str(j+1) + ']')
        ...
        for k in range(nplot-1):                    
            ...
            ax=axs2[k,i]
            for j in range(k,nplot):
                #jj=kk-j
                covar=np.mean(binned[j]*binned[k],axis=1)
                #print(jj,kk,covar[1],stdevs[jj,1],stdevs[kk,1],stdevs[jj,1]*stdevs[kk,1])
                #print((binned[jj]*binned[kk])[:,1])
                #print(covar.shape)
                ax.plot((covar/stdevs[j]/stdevs[k])[::ev], marker='.',linewidth=1, label=r'$\mathrm{ch}$[' + str(k+1)+','+str(j+1)+ ']')
            ax.legend(loc='upper left', ncol=2)
            ax.grid(linewidth=1, which='both', color='gray', linestyle='dotted')
            ax.set_xlabel("sample")
            ax.set_ylabel(r"stdev")
